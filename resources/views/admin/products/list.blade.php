                                        ...
                                        @if($products->isNotEmpty())
											...
											<td>
												...
												<form action="{{ route('products.delete', $product->id) }}" method="POST" id="deleteForm-{{ $product->id }}">
          											@csrf
    												@method('DELETE')
    												<a href="#" class="text-danger w-4 h-4 mr-1" onclick="confirmDelete(event, this)">
        											<svg class="filament-link-icon w-4 h-4 mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
...
<script>
    function confirmDelete(event, link) {
        ...
        var confirmDelete = confirm('Are you sure you want to delete this product?'); // Display the confirmation dialog

        if (confirmDelete) {
            $(link).closest('form').submit(); // If confirmed, submit the form
        }
    }
</script>
@endsection